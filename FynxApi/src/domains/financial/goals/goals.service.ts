...
export class GoalsService {
  ...
  static async getGoalsData(userId: number): Promise<GoalsData> {
    ...
    try {
      ...
      const spendingGoals = goalRows.map((row: SpendingGoalRow) => formatSpendingGoalFromDB(row));
      ...
      const budgets = budgetRows.map((row: BudgetRow) => formatBudgetFromDB(row));
  ...
  static async createSpendingGoal(data: CreateSpendingGoalRequest & { userId: number }): Promise<SpendingGoal> {
    const db = database;
    const now = new Date().toISOString();

    try {
      const { userId, ...goalData } = data;
      const goalType = goalData.goalType ?? 'spending';
  ...
  static async updateSpendingGoal(id: string, data: UpdateSpendingGoalRequest & { goalType?: string }): Promise<SpendingGoal | null> {
    const db = database;
    const now = new Date().toISOString();
  ...
  static async getBudgets(userId: number): Promise<Budget[]> {
    ...
    try {
      ...
      return rows.map((row: BudgetRow) => formatBudgetFromDB(row));
    } catch (error) {
      throw error;
    }
  }
  ...
  static async createBudget(data: CreateBudgetRequest & { userId: number }): Promise<Budget> {
    const db = database;
    const now = new Date().toISOString();
